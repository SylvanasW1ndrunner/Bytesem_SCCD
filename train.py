...
def train(args):
    ...
    for epoch in range(args.epochs):
        ...
        for data1, data2, label in tqdm(train_loader, desc=f"Epoch {epoch+1}/{args.epochs}"):
            ...
            loss = criterion(output1, output2, label)
            loss.backward()
            last_layer_grad = model.final_linear.weight.grad
            # if last_layer_grad is not None:
            #     # 计算梯度的绝对值总和，看它是否为0
            #     grad_sum = last_layer_grad.abs().sum().item()
            #     print(f"\n[诊断] 最后一个线性层的梯度总和: {grad_sum}")
            #     if grad_sum == 0.0:
            #         print("  🔥 警告: 梯度为0！模型权重没有收到任何更新信号。")
            # else:
            #     print("\n[诊断] 🔥 致命错误: 最后一个线性层的梯度为 None！梯度根本没有被计算。")
            # --- 诊断结束 ---
            optimizer.step()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--processed_dir', type=str, required=False, help='预处理好的图数据(.pt)文件夹',default='GNNdata/ptdata')
    ...
    parser.add_argument('--model_path', type=str, help='模型保存路径',default='models/model')
